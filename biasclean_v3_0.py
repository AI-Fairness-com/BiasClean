...
if __name__ == "__main__":
    print(__doc__)
    print("\nUsage:")
    print("  Interactive: run_biasclean_interactive()")
    print("  Quick audit: quick_audit('data.csv', domain='justice')")
    print("  Full API:    pipeline = UniversalBiasClean(domain='justice')")
    print("               results = pipeline.process_dataset(df=dataframe)")


# ============================================================================
# COLAB INTERACTIVE TEST CELL - Run this in Google Colab
# ============================================================================
...
try:
    ...
    print("="*80)
    print("BiasClean v3.0 - Google Colab Interactive Mode")
    print("="*80)
    print()
    ...
    if not uploaded:
        ...
        print("-" * 80)
        domains = {
            "1": ("justice", "Criminal justice, recidivism, bail"),
            "2": ("health", "Healthcare, diagnosis, treatment"),
            "3": ("finance", "Credit scoring, loans, insurance"),
            "4": ("hiring", "Recruitment, job offers, promotions"),
            "5": ("education", "Admissions, grading, outcomes"),
            "6": ("business", "Funding, contracts, procurement"),
            "7": ("governance", "Elections, appointments, representation")
        }

        for num, (key, desc) in domains.items():
            print(f"  {num}. {key.title():12} - {desc}")

        domain_choice = input("\nEnter choice (1-7, default=1): ").strip() or "1"
        domain_key = domains.get(domain_choice, domains["1"])[0]
        print(f"✅ Selected: {domain_key.title()}")
        print()

        # Step 3: Show columns
        print("📊 STEP 3: Select target column")
        print("-" * 80)
        print("Your columns:")
        for i, col in enumerate(df.columns, 1):
            uniq = df[col].nunique()
            dtype = str(df[col].dtype)
            sample = str(df[col].iloc[0])[:20] if len(df) > 0 else "N/A"
            print(f"  {i:2}. {col:30} (unique: {uniq:5}, type: {dtype:10}, sample: {sample})")

        target_input = input("\nEnter target column name (or press Enter for auto): ").strip()
        target_column = target_input if target_input else None
        print()
        ...
        print("-" * 80)
        print("  1. Audit-First (Recommended) - Check safety before fixing")
        print("  2. Audit-Only - Just diagnose, don't change data")
        print("  3. Legacy - Original v2.7 behavior (skip audit)")
        ...
        # Step 5: SVM option
        print("🤖 STEP 5: SVM Fairness Enforcement")
        print("-" * 80)
        print("SVM adds model-level fairness (experimental, may introduce trade-offs)")
        svm_choice = input("Enable SVM? (y/N): ").strip().lower()
        enable_svm = (svm_choice == 'y')
        print(f"✅ SVM: {'ENABLED' if enable_svm else 'DISABLED (default)'}")
        print()
        ...
        if mode in ["audit_first", "audit_only"]:
            if "audit" in results:
                ...
                print()
                print(f"Bias Score: {audit['baseline_fairness']['aggregate_bias']:.3f}")
                print(f"Records: {audit['data_quality']['n_records']:,}")
                print(f"Vulnerable Groups: {audit['vulnerable_subgroups']['count']}")
                ...
                if mode == "audit_only":
                    print("\n🔍 AUDIT-ONLY MODE - No mitigation performed")
                elif not rec.get('proceed', True):
                    print("\n🛑 MITIGATION BLOCKED by audit")
                else:
                    print("\n✅ Mitigation completed (if audit approved)")

        print()
        print("💾 Results saved to: biasclean_results/")
        print("="*80)
        print()
        print("To download results:")
        print("1. Open Files panel (folder icon on left)")
        print("2. Navigate to biasclean_results/")
        print("3. Right-click files to download")
        print("="*80)

except ImportError:
    # Not in Colab - show usage instructions
    print(__doc__)
    print("\nThis file is ready to use. In Google Colab, it will automatically")
    print("prompt for CSV upload and run interactively.")
    print()
    print("In regular Python:")
    print("  from biasclean_v3_production import UniversalBiasClean")
    print("  pipeline = UniversalBiasClean(domain='justice')")
    print("  results = pipeline.process_dataset(df=your_data)")

# BiasClean — User Guide (Health Domain)

This guide covers running BiasClean's Health-domain pipeline (v3.10.10) as a
non-technical user, on your own dataset, and reading the resulting report. It assumes no
coding background.

## What this tool does

BiasClean audits a health dataset for disparities in outcome rates across groups
...
`LIMITATIONS_HEALTH.md` for the full scope of what this tool is (and isn't) for.
...
docker run -it \
  -v "/path/to/your/data:/app/Health_Real Datasets" \
...
2. **Target column** — this is now required (auto-detection was removed after it proved
   unreliable). Type the exact name of your outcome column (e.g. readmitted_30d, heart_disease, diagnosis_outcome).
...
6. **SVM fairness enforcement** — leave this at the default (N/disabled) unless you specifically
   ...
   `LIMITATIONS_HEALTH.md`), so results can vary slightly run to run.

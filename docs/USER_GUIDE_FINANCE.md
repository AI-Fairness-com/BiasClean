# BiasClean — User Guide (Finance Domain)

This guide covers running BiasClean's Finance-domain pipeline (v3.10.10) as a
non-technical user, on your own dataset, and reading the resulting report. It assumes no
coding background.

## What this tool does

BiasClean audits a finance dataset for disparities in outcome rates across groups
...
`LIMITATIONS_FINANCE.md` for the full scope of what this tool is (and isn't) for.
...
docker run -it \
  -v "/path/to/your/data:/app/Finance_Real Datasets" \
...
2. **Target column** — this is now required (auto-detection was removed after it proved
   unreliable). Type the exact name of your outcome column (e.g. loan_approved, CreditRisk, credit_decision).
...
6. **SVM fairness enforcement** — leave this at the default (N/disabled) unless you specifically
   ...
   `LIMITATIONS_FINANCE.md`), so results can vary slightly run to run.

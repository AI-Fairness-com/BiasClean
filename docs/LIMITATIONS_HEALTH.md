# BiasClean — Limitations Note (Health Domain)

**One-page scope statement, to be read alongside `DISCLAIMER.md`.**

## What BiasClean's Health-domain pipeline is

A screening tool. It flags statistically measurable disparities in outcome rates between
groups in a health dataset, and offers one specific, transparent, auditable way of
...
- **Not validated beyond what's been tested.** The Health-domain pipeline has been
  validated against real, publicly sourced data:
  - **Diabetes 130-US Hospitals** — target column `readmitted_30d`, 99,492 → 99,492 rows
  - **Heart Disease (UCI Cleveland)** — target column `heart_disease (binarized from the original 5-category target column)`, 303 → 303 rows
...
## Known operating constraints (Health domain, as of v3.10.10)

- Heart Disease's 303-row size is well below the other validated Health dataset (99,492 rows); small-sample results should be reviewed with extra care, particularly for any subgroup near the minimum group-size floor.
- Only two conditions (diabetic readmission, heart disease) have been validated. Other clinical outcome types have not been specifically tested.

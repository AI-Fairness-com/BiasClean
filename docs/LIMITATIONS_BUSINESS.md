# BiasClean — Limitations Note (Business Domain)

**One-page scope statement, to be read alongside `DISCLAIMER.md`.**

## What BiasClean's Business-domain pipeline is

A screening tool. It flags statistically measurable disparities in outcome rates between
groups in a business dataset, and offers one specific, transparent, auditable way of
...
- Age (only protected attribute currently mapped in this domain)
...
- **Not validated beyond what's been tested.** The Business-domain pipeline has been
  validated against real, publicly sourced data:
  - **Bank Marketing (bank-full)** — target column `y`, 45,211 → 45,211 rows
...
## Known operating constraints (Business domain, as of v3.10.10)

- Business's ontology currently has no MaritalStatus protected-attribute category, even though this dataset includes a marital column — an accepted scope limitation (open item 1), not a defect, per a deliberate policy decision not to add an 8th protected-attribute category at this stage.
- Only one dataset (bank telemarketing) has been validated for this domain. Other Business use cases — insurance pricing, retail credit, service pricing — have not been specifically tested.

# BiasClean — Limitations Note (Finance Domain)

**One-page scope statement, to be read alongside `DISCLAIMER.md`.**

## What BiasClean's Finance-domain pipeline is

A screening tool. It flags statistically measurable disparities in outcome rates between
groups in a finance dataset, and offers one specific, transparent, auditable way of
...
- SocioeconomicStatus (weighted highest in this domain, 0.30)
- Ethnicity
- Gender
- Age
...
- **Not validated beyond what's been tested.** The Finance-domain pipeline has been
  validated against real, publicly sourced data:
  - **HMDA (Washington State, race cut)** — target column `loan_approved`, 180,056 → 180,056 rows
  - **German Credit (Statlog)** — target column `CreditRisk`, 1,000 → 1,000 rows
...
## Known operating constraints (Finance domain, as of v3.10.10)

- The Race-vs-Hispanic-origin-Ethnicity distinction (HMDA's derived_race and derived_ethnicity) is not separately modeled; only one wins the single Ethnicity slot. Treat this as a known taxonomy scope limit, not a data-quality issue.
- German Credit's ForeignWorker column was not originally recognized as a MigrationStatus proxy ('foreign' was absent from the keyword list); this gap was closed in v3.10.6.
- Only two real datasets have been validated for this domain (mortgage lending and consumer credit). Other Finance use cases — insurance underwriting, fraud scoring, investment products — have not been specifically tested.

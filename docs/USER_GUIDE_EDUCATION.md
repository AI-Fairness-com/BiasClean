# BiasClean — User Guide (Education Domain)

This guide covers running BiasClean's Education-domain pipeline (v3.10.10) as a
non-technical user, on your own dataset, and reading the resulting report. It assumes no
coding background.

## What this tool does

BiasClean audits an education dataset for disparities in outcome rates across groups
...
`LIMITATIONS_EDUCATION.md` for the full scope of what this tool is (and isn't) for.
...
docker run -it \
  -v "/path/to/your/data:/app/Education_Real Datasets" \
...
2. **Target column** — this is now required (auto-detection was removed after it proved
   unreliable). Type the exact name of your outcome column (e.g. pass_bar, completed, graduation_outcome).
...
6. **SVM fairness enforcement** — leave this at the default (N/disabled) unless you specifically
   ...
   `LIMITATIONS_EDUCATION.md`), so results can vary slightly run to run.

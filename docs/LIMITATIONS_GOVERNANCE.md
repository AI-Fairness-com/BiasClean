# BiasClean — Limitations Note (Governance Domain)

**One-page scope statement, to be read alongside `DISCLAIMER.md`.**

## What BiasClean's Governance-domain pipeline is

A screening tool. It flags statistically measurable disparities in outcome rates between
groups in a governance dataset, and offers one specific, transparent, auditable way of
...
- **Not validated beyond what's been tested.** The Governance-domain pipeline has been
  validated against real, publicly sourced data:
  - **ACS Public Coverage (Washington, 2018)** — target column `PUBCOV`, 24,312 → 24,377 rows
...
## Known operating constraints (Governance domain, as of v3.10.10)

- Only Washington State's 2018 ACS extract has been validated. Other states, years, or ACS outcome variables (beyond PUBCOV) have not been specifically tested.
- Census/ACS PUMS variable codes are recognized for a specific set of columns (AGEP, RAC1P, PINCP, POVPIP, PUMA, CIT, NATIVITY, DIS); ST and HISP are deliberately excluded from auto-recognition (open item 8, partial by design).

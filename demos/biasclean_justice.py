# -*- coding: utf-8 -*-
"""Another copy of BiasClean_Justice.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1-EkonJn83JwaaPS-WaB_XTslMjOPlFdz
...
class BiasClean:
    ...
    def _industry_smote_rebalancing(self, df: pd.DataFrame, diagnostic_results: dict, max_data_loss: float = 0.08) -> pd.DataFrame:
        """
        JUSTICE-DOMAIN ENHANCED: Effort-proportional weight optimization
        EFFORT ALLOCATION: Ethnicity (50%) → Age (30%) → Gender (10%) → Trade-offs (10%)
        """
        logger.section_header("JUSTICE DOMAIN OPTIMIZATION - EFFORT PROPORTIONAL")
        ...
        # JUSTICE PRIORITY: Sort by weight and allocate effort
        justice_priority = []
        for feature in significant_features:
            weight = self.domain_weights.get(feature, 0)
            justice_priority.append((feature, weight))

        justice_priority.sort(key=lambda x: x[1], reverse=True)

        # Calculate effort allocation based on weights
        total_weight = sum(weight for _, weight in justice_priority)
        effort_allocation = {}
        for feature, weight in justice_priority:
            effort_pct = (weight / total_weight) * 100
            effort_allocation[feature] = effort_pct

        logger.info(f"Justice effort allocation: {effort_allocation}")
        ...
        if significant_features:
            ...
            for feature in significant_features:
                ...
                logger.info(f"   {feature} (multi-objective): {initial_disparities[feature]:.4f} → {final_disparity:.4f} ({improvement:+.1f}%)")
        ...
        if final_validation['success']:
            logger.success("✅ JUSTICE OPTIMIZATION SUCCESS")
            for feature, improvement in final_validation['improvements'].items():
                weight = self.domain_weights.get(feature, 0)
                effort = effort_allocation.get(feature, 0)
                logger.info(f"   {feature} (weight: {weight}, effort: {effort:.1f}%): {improvement['before']:.4f} → {improvement['after']:.4f} ({improvement['change']:+.1f}%)")
        else:
            logger.warning("⚠️  Justice optimization suboptimal - keeping improvements")
        ...
        logger.section_header("JUSTICE EFFORT-PROPORTIONAL RESULTS")
    ...
    def _multi_objective_optimization(self, df: pd.DataFrame, features: list, initial_disparities: dict, max_iterations: int = 10) -> pd.DataFrame:
        ...
        logger.info("🚀 MULTI-OBJECTIVE OPTIMIZATION: Simultaneous feature optimization")

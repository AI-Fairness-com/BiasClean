...
class HierarchicalMapper:
    ...
    def __init__(self):
        ...
        self.justice_ontology = {
            "outcome_patterns": ["recid", "reoffend", "rearrest", "outcome", "target", "label", "violat"],
            "contextual_patterns": {
...
class ReportGenerator:
    ...
    def generate_html_report(self, results: Dict, output_file: str = "biasclean_report.html"):
        ...
        <body>
            <div class="header">
                <h1>Universal BiasClean Analysis Report</h1>
                <p>Generated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}</p>
                <p>Domain: Justice | Version: 2.0</p>
            </div>
        """
...
class UniversalBiasClean:
    ...
    def __init__(self, domain: str = "justice", jurisdiction: Optional[str] = None):
        ...
        # Justice domain weights (from COMPAS)
        domain_weights = {
            "Ethnicity": 0.25,
        ...
        self.engine = BiasCleanEngine(domain_weights)
...
def run_interactive_pipeline():
    ...
    print(f"\n{'='*80}")
    print("INITIALIZING UNIVERSAL BIASCLEAN PIPELINE")
    print(f"{'='*80}\n")

    pipeline = UniversalBiasClean(domain="justice", jurisdiction="US")
...
def quick_start_csv(file_path: str, target_column: str = None):
    """
    ...
    Returns:
        Pipeline results
    """
    print(f"QUICK START: Running BiasClean on {file_path}...\n")
    ...
    <div style='background: linear-gradient(135deg, #667eea 0%, #764ba2 100%);
        ...
        <div style='background: rgba(255,255,255,0.1); padding: 15px; border-radius: 5px;'>
            ...
            <p style='font-size: 18px;'>
                Improvement: <strong>{
                    ((diagnostics.get('initial_bias_score', 0) - diagnostics.get('final_bias_score', 0)) /
                    diagnostics.get('initial_bias_score', 1) * 100) if diagnostics.get('initial_bias_score', 0) > 0 else 0
                :+.1f}%</strong>
            </p>
        </div>

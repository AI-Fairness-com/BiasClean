...
def generate_pdf(session_id):
    ...
    Returns helpful error since WeasyPrint isn't installed.
    """
    if request.method == 'OPTIONS':
        return jsonify({'status': 'ok'}), 200
    ...
    try:
        ...
        # Return helpful message about PDF generation
        return jsonify({
            'status': 'info',
            'message': 'PDF generation requires WeasyPrint installation.',
            'instructions': 'Add "weasyprint==61.0" to requirements.txt and redeploy',
            'available_downloads': {
                'html_report': f'{BASE_URL}/download/{report_filename}',
                'cleaned_data': f'{BASE_URL}/download/corrected_{session_id}.csv'
            }
        }), 200
        
    except Exception as e:
        app.logger.error(f"PDF endpoint error: {str(e)}")
        return jsonify({'error': 'PDF service error', 'details': str(e)[:100]}), 500

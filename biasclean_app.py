...
FIXED VERSION: HTML report with embedded visualizations and working download links
"""
...
# ============================================================================
# HTML REPORT GENERATOR - FIXED VERSION WITH ABSOLUTE URLS
# ============================================================================
def generate_html_report(parser, viz_base64, domain, session_id, pipeline_output, 
    ...
    """Generate HTML report with absolute URLs for download links"""
...
'''
    ...
    # Close table and add download buttons with absolute URLs
    html += f'''
            </tbody>
        </table>
        ...
        <div style="text-align: center; margin: 40px 0;">
            ...
            <a href="{base_url}/download/report_{session_id}.html" 
               class="download-btn"
               download="biasclean_report_{session_id}.html">
                📈 Download Full Report (HTML)
            </a>
            <a href="{base_url}/download/corrected_{session_id}.csv" 
               class="download-btn"
               style="background: #48bb78;"
               download="cleaned_data_{session_id}.csv">
                📊 Download Cleaned Dataset (CSV)
            </a>
            <p style="margin-top: 15px; color: #666; font-size: 0.9em;">
                <em>Click to download • Right-click "Save Link As..." for alternate download</em>
            </p>
        </div>
...
def analyze():
    ...
    try:
        ...
        # GENERATE HTML REPORT WITH ABSOLUTE URLs
        ...
        try:
            ...
            app.logger.info(f"HTML report saved: {report_path}, size: {os.path.getsize(report_path)} bytes")
...
def view_report(filename):
    """View HTML report in browser instead of downloading"""
    try:
        ...
        # Serve HTML files for viewing in browser
        if filename.endswith('.html'):
            return send_file(
                file_path,
                mimetype='text/html',
                as_attachment=False  # KEY: Display in browser, don't download
            )
        else:
...
def download(filename):
    ...
    try:
        ...
        if not os.path.exists(file_path):
            app.logger.error(f"File not found: {file_path}")
            app.logger.error(f"Available files in {app.config['UPLOAD_FOLDER']}: {[f for f in os.listdir(app.config['UPLOAD_FOLDER']) if filename.split('_')[0] in f]}")
            return jsonify({'error': 'File not found'}), 404

...
MINIMAL FIX: HTML report generation WITHOUT breaking visualization capture
"""
...
def analyze():
    ...
    try:
        ...
        # RUN PIPELINE WITH OUTPUT CAPTURE (ORIGINAL WORKING CODE)
        app.logger.info(f"Starting pipeline for domain: {domain}")
        pipeline = UniversalBiasClean(domain=domain)
        
        # Disable visualization and file saving (ORIGINAL WORKING CODE)
        ...
        # Create temp directory for potential visualizations
        ...
        # CAPTURE ALL CONSOLE OUTPUT (ORIGINAL WORKING CODE)
        ...
        # PARSE THE CAPTURED OUTPUT (ORIGINAL WORKING CODE)
        parser = PipelineOutputParser(pipeline_output)
        
        # Collect any generated visualizations (SIMPLE VERSION)
        viz_base64 = capture_visualizations(viz_temp_dir)
        app.logger.info(f"Found {len(viz_base64)} visualizations")
        
        # Extract metrics (ORIGINAL WORKING CODE)
        ...
        # Save corrected file (ORIGINAL WORKING CODE)
        ...
        # GENERATE HTML REPORT (NEW FEATURE)
        ...
        try:
            ...
            # Fallback to TXT report (ORIGINAL WORKING CODE)
        ...
        # Return response (UPDATED to include visualizations)
        response = {
            'detection': {
                'n_rows': int(len(df)),
